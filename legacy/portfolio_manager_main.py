...
def validate_period(period):
    """Validate period for intraday data"""
...
║    • Interday: 1d, 5d, 1wk, 1mo, 3mo                             ║
║ 4. Time Range:                                                   ║
║    • Intraday: Period only (e.g., 5d, 30d, max 60d)              ║
...
║   5m  - 5 minute bars    60m - 60 minute bars                    ║
║   90m - 90 minute bars   1h  - 1 hour bars                       ║
║                                                                  ║
...
def portfolio_creation_flow(user_id: int, pm: PortfolioManager):
    ...
    try:
        ...
        # Step 3: Interval
        print("\nAvailable intervals:")
        print("Intraday: 1m, 2m, 5m, 15m, 30m, 60m, 90m, 1h")
        ...
        if data_type == 'intraday':
            ...
            while True:
                period = input("Enter period (e.g., 5d, 30d, max 60d) or 'help': ").strip()
